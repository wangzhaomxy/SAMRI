        ...
        for prompt in prompts:
            for step, (embedding, masks, ori_size) in enumerate(tqdm(train_loader)):
                ...
                if prompt == "point":
                    batch_input = [
                        {'image': image.squeeze().to(device),
                            'point_coords':resize_transform.apply_coords_torch(torch.as_tensor(np.array([gen_points(mask.squeeze(-3).numpy())]), device=device), original_size=ori_size),
                ...
                if prompt == "bbox":
                    batch_input = [
                        {'image': image.squeeze().to(device),
                            'boxes':resize_transform.apply_boxes_torch(torch.as_tensor(np.array([gen_bboxes(mask.squeeze(-3).numpy())]), device=device), original_size=ori_size),
                            'original_size':ori_size
                            } 
                        for image, mask, ori_size in zip(embedding, masks, ori_size)
                    ]
